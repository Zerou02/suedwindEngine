...
const update = () => {
  ...
  if ("w" in pressedKeys) shiftWorld(pos.new(0, -1));
  if ("s" in pressedKeys) shiftWorld(pos.new(0, 1));
  if ("a" in pressedKeys) shiftWorld(pos.new(-1, 0));
  if ("d" in pressedKeys) shiftWorld(pos.new(1, 0));
};
