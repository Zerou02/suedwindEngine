...
export class SpriteSheetSelection {
    ...
    async initializeSpriteSheetData() {
        await fetch(`${gAssetPath}/spritesheets/${this.sheetname}.json`)
            ...
            const pageCount = createLabel({ x: 60, y: baseDimensions.h + 50, w: 25, h: 50 }, `${this.currentPage + 1 + "/" + this.numberPages}`, "pageCount");
            console.log("A");
    ...
    loadPage(number) {
        ...
        let pageLabel = this.scene.menuItems["pageCount"];
        `${this.currentPage + 1 + "/" + this.numberPages}`;
        pageLabel.textContent = `${this.currentPage + 1 + "/" + this.numberPages}`;
        let ctx = this.sheetLayer.canvas.getContext("2d");
        ctx.clearRect(0, 0, this.sheetLayer.dimensions.w, this.sheetLayer.dimensions.h);
        ctx.drawImage(this.img, this.sheetData.pageSize * this.sheetData.tileSize * number, 0, this.sheetData.pageSize * this.sheetData.tileSize, this.sheetData.sizeY, 0, 0, this.sheetLayer.dimensions.w, this.sheetLayer.dimensions.h);
    }
}
